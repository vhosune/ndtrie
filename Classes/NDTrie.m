...
#include <string.h>
  ...
  #error This file cannot be compiled with ARC enabled (-fno-objc-arc)
#endif


static NSString * const kPListPListElementName  = @"plist";
static NSString * const kArrayPListElementName  = @"array";
static NSString * const kStringPListElementName = @"string";
...
static BOOL removeObjectForKey(struct trieNode * aNode, id aKey, NSUInteger anIndex, BOOL * aFoundNode, NSUInteger (* aKeyComponentFunc)(id, NSUInteger, BOOL *) );
static NSUInteger removeAllChildren(struct trieNode * aNode);
static NSUInteger removeChild(struct trieNode * aRoot, id aPrefix, NSUInteger (* aKeyComponentFunc)(id, NSUInteger, BOOL *) );
static BOOL setObjectForKey(struct trieNode * aNode, id anObject, id aKey, NSUInteger (* aKeyComponentFunc)(id, NSUInteger, BOOL *) );
static BOOL forEveryObjectFromNode(struct trieNode * aNode, BOOL (* aFunc)(NSString *, id, void *), void * aContext);
static void forEveryObjectWithBlockFromNode(struct trieNode * aNode, void (^ aBlock)(id, BOOL *), BOOL * aStop);
static BOOL nodesAreEqual(struct trieNode * aNodeA, struct trieNode * aNodeB);
static struct trieNode * copyNode(struct trieNode * aNode);
static BOOL getObjectsFunc(NSString * key, id anObject, void * aContext);
...
enum NDTriePListElement
{
  NDTriePListElementNone,
  NDTriePListElementArray,
  NDTriePListElementDictionary
};
...
@interface NDTrieBuilder : NSObject <NSXMLParserDelegate>
{
  ...
  enum NDTriePListElement _foundRootElement;
...
static NSUInteger keyComponentForString(NSString * anObject, NSUInteger anIndex, BOOL * anEnd)
{
...

    {
      ...
      if(![theString isKindOfClass:[NSString class]])
      {
        @throw [NSException exceptionWithName:NSInvalidArgumentException
                                       reason:[NSString stringWithFormat:@"An attempt was made to add and object of class %@ to a NDTrie", [theString class]]
                                     userInfo:nil];
      }
...
- (id)initWithDictionary:(NSDictionary *)aDictionary
{
  ...
  if(self)
  {
...
#endif
    {
      ...
      {
        @throw [NSException exceptionWithName:NSInvalidArgumentException
                                       reason:[NSString stringWithFormat:@"An attempt was made to add and object of class %@ to a NDTrie", [theKey class]]
                                     userInfo:nil];
      }
...
static BOOL _addToArrayFunc(NSString * aString,  id anObject, void * anArray)
{
...
{
  struct getObjectsCountData theData = {
    0, aCount, copy, aBuffer
  };
...
- (void)enumerateObjectsUsingFunction:(BOOL (*)(NSString * key))aFunc
{
  forEveryObjectFromNode([self root], (BOOL (*)(NSString *, id, void *))aFunc, NULL);
}
...
- (void)enumerateObjectsForKeysWithPrefix:(NSString *)aPrefix usingFunction:(BOOL (*)(id obj))aFunc
{
  struct trieNode * theNode = [self root];
  ...
  if(theNode != nil)
  {
    forEveryObjectFromNode(theNode, (BOOL (*)(NSString *, id, void *))aFunc, NULL);
  }
}


- (void)enumerateObjectsUsingFunction:(BOOL (*)(NSString * key, id obj, void * context))aFunc context:(void *)aContext
{
  forEveryObjectFromNode([self root], aFunc, aContext);
}


- (void)enumerateObjectsForKeysWithPrefix:(NSString *)aPrefix usingFunction:(BOOL (*)(NSString * key, id obj, void * context))aFunc context:(void *)aContext
{
  struct trieNode * theNode = [self root];
...
struct testData
{
  ...
  BOOL (^ block)(NSString *, id, BOOL *);
};

BOOL testFunc(NSString * aKey, id anObject, void * aContext)
{
  struct testData * theData = (struct testData *)aContext;
  BOOL theTestResult = NO;

  if(theData->block(aKey, anObject, &theTestResult) )
  {
...
- (NSArray *)everyObjectPassingTest:(BOOL (^)(NSString *, id, BOOL *))aPredicate
{
  struct testData theData = {
    [NSMutableArray array], aPredicate
  };
...
- (NSArray *)everyObjectForKeyWithPrefix:(NSString *)aPrefix passingTest:(BOOL (^)(NSString * key, id object, BOOL * stop))aPredicate
{
  struct testData theData = {
    [NSMutableArray array], aPredicate
  };
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)aState objects:(id *)aStackbuf count:(NSUInteger)aLen
{
  ...
  if(aState->state == 0)
  {
    ...
    struct getObjectsCountData theData = {
      0, theCount, assign, (id *)malloc(theCount * sizeof(id) )
    };
...
static BOOL addTrieFunc(NSString * aString, id anObject, void * aContext)
{
  NDMutableTrie * theTrie = (NDMutableTrie *)aContext;

  if(anObject)
  {
    [theTrie setObject:anObject forKey:aString];
  }
  else
  {
...
{
  [aTrie enumerateObjectsUsingFunction:addTrieFunc context:(void *)self];
}
...
- (void)setObject:(id)anObject forKeyedSubscript:(NSString *)aString
{
  ...
  {
    @throw [NSException exceptionWithName:NSInvalidArgumentException
                                   reason:@"The key subscript must of of kind NSString"
                                 userInfo:nil];
  }
...
- (id)nextObject
{
  return _index < _count ? _everyObject[_index++] : nil;
}


- (NSArray *)allObjects
{
  return [NSArray arrayWithObjects:_everyObject + _index count:_count - _index];
}
...
{
  if(_foundRootElement == NDTriePListElementNone)
  {
    if([anElementName isEqualToString:kArrayPListElementName])
    {
      _foundRootElement = NDTriePListElementArray;
    }
...
- (void)parser:(NSXMLParser *)aParser didEndElement:(NSString *)anElementName namespaceURI:(NSString *)aNamespaceURI qualifiedName:(NSString *)aQName
{
  if(_foundRootElement != NDTriePListElementNone)
  {
    if([anElementName isEqualToString:kArrayPListElementName])
    {
      _foundRootElement = NDTriePListElementNone;
    }
    else if([anElementName isEqualToString:kStringPListElementName])
    {
...
static struct trieNode * findNode(struct trieNode * aNode, id aKey, NSUInteger anIndex, BOOL aCreate, struct trieNode ** aParent, NSUInteger * anPosition, NSUInteger (* aKeyComponentFunc)(id, NSUInteger, BOOL *) )
{
  ...
  {
    @throw [NSException exceptionWithName:NSInvalidArgumentException
                                   reason:@"objectForKey: key cannot be nil"
                                 userInfo:nil];
  }
    ...
    if(aNode->children != NULL)
    {
      ...
      if(theIndex >= aNode->count || aNode->children[theIndex]->key != theKeyComponent)
      {
        if(aCreate)
        {
          ...
          {
            aNode->size <<= 1;
            size_t resize = MAX(aNode->size * sizeof(struct trieNode *), sizeof(struct trieNode *));
            aNode->children = (struct trieNode **)reallocf(aNode->children, resize);
    ...
    if(theIndex < aNode->count)
    {
      if(aNode->children[theIndex]->key == theKeyComponent)
      {
        if(removeObjectForKey(aNode->children[theIndex], aKey, anIndex + 1, aFoundNode, keyComponentForString) )
        {
          ...
          [aNode->children[theIndex]->skey release];
...
BOOL setObjectForKey(struct trieNode * aNode, id anObject, id aKey, NSUInteger (* aKeyComponentFunc)(id, NSUInteger, BOOL *) )
{
  ...
  if(anObject == nil)
  {
    @throw [NSException exceptionWithName:NSInvalidArgumentException
                                 ...
                                 userInfo:[NSDictionary dictionaryWithObject:aKey
                                                                      forKey:@"key"]];
  }
...
BOOL forEveryObjectFromNode(struct trieNode * aNode, BOOL (* aFunc)(NSString *, id, void *), void * aContext)
{
  BOOL theContinue = YES;

  if(aNode->object != nil)
  {
    theContinue = aFunc(aNode->skey, aNode->object, aContext);
  }
